...
#define R(p, i, n) C(phnumGet(p, i), n)

// Oczekiwany brak i-tego numeru w strukturze PhoneNumbers
#define Q(p, i) Z(phnumGet(p, i))
...
void *__wrap_reallocarray(void *ptr, size_t nmemb, size_t size) {
    UNRELIABLE_ALLOC(ptr, nmemb * size, __real_reallocarray(ptr, nmemb, size),
                     "reallocarray");
}
...
static int alloc_fail(void) {
    unsigned fail, pass;
    for (fail = 0, pass = 0, fail_counter = 1; fail < 3 && pass < 3;
         ++fail_counter) {
        call_counter = 0;
        alloc_counter = 0;
        free_counter = 0;
        ...
        if (alloc_counter != free_counter ||
            (visited_point & 04444444444) != 0) {
            fprintf(stderr,
                    "fail_counter %u, alloc_counter %u, free_counter %u, "
                    "function_name %s, visited_point %o\n",
                    fail_counter, alloc_counter, free_counter, function_name,
                    visited_point);
            ++fail;
        }
...
static const test_list_t test_list[] = {
        TEST(empty_struct),      TEST(no_forward),
        TEST(wrong_arguments),   TEST(malicious_arguments),
        TEST(breaking_struct),   TEST(long_numbers),
        TEST(copy_arguments),    TEST(two_structs),
        TEST(delete_null),       TEST(persistent_results),
        TEST(forward_overwrite), TEST(remove_forward),
        TEST(various_ops),       TEST(many_ops),
        TEST(very_long),         TEST(many_remove),
        TEST(add_remove),        TEST(alloc_fail),
};
